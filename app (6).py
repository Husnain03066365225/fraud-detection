    ...
    with tab2:
        if st.button("🚀 Train Improved Model"):
            with st.spinner("Training..."):
                # Drop only 'Class' (keep Time, V1-V28, Amount)
                ...
                st.session_state.columns = X.columns.tolist()   # Save exact columns
    ...
    with tab3:
        ...
        if 'model' not in st.session_state:
            ...
            if st.button("🔍 Predict if this is Fraud"):
                # Create input with exact same columns as training
                input_dict = {col: 0.0 for col in st.session_state.columns}
                
                # Fill V1 to V28
                for i in range(28):
                    input_dict[f"V{i+1}"] = input_values[i]
